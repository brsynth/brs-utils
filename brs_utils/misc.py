...
from subprocess import run, DEVNULL  # nosec
...
def subprocess_call(
    cmd: str, stdout=None, stderr=None, logger: Logger = getLogger(__name__)
) -> int:
    if stdout is None:
    ...
    try:
        CPE = run(cmd.split(), stdout=stdout, stderr=stderr, shell=False)  # nosec
        logger.debug(CPE)
        return CPE
    except OSError as e:
        logger.error(e)
...
def check_nb_args(*args, f_name, nb_args):
    if len(args) < nb_args:
        raise TypeError(
            f_name + " missing " + str(nb_args) + " required positional argument"
        )
