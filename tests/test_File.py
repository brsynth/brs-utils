...
from brs_utils.file import compare_dir
from tempfile import NamedTemporaryFile, TemporaryDirectory, mkdtemp
from filecmp import cmp, dircmp
from pathlib import Path
from hashlib import sha256
from json import dumps as json_dumps
from os import path as os_path, remove, stat
...
class Test_File(TestCase):
    ...
    DOWNLOAD_TARGZ_URL = (
        "https://github.com/brsynth/brs-utils/raw/master/tests/data/data.tar.gz"
    )
    DOWNLOAD_ZIP_URL = (
        "https://github.com/brsynth/brs-utils/raw/master/tests/data/100l_file.txt.zip"
    )
    DOWNLOAD_HASH = "451f23622ee6df7b96648039db374e7baa8c41495e4dbf91dbf232950db65a13"
    DOWNLOAD_HASHES = {
        "test_rpSBML.py": "2031eefaf7305428e10f5a3c6ab485085d69979b8be6a1e3b2e375349ec7b9bd",
    ...
    DATA_FOLDER = os_path.join(os_path.dirname(os_path.realpath(__file__)), "data")
    ...
    def test_download_with_filename(self):
        download_f = download(Test_File.DOWNLOAD_TARGZ_URL)
        self.assertEqual(
            sha256(Path(download_f).read_bytes()).hexdigest(), Test_File.DOWNLOAD_HASH
        )
        remove(download_f)
    ...
    def test_download_without_filename(self):
        ...
        self.assertEqual(
            sha256(Path(outfile).read_bytes()).hexdigest(), Test_File.DOWNLOAD_HASH
        )
        remove(outfile)
    ...
    def test_compress_tar_gz_file(self):
        ...
        oufile = compress_tar_gz(
            infile, NamedTemporaryFile().name + ".tar.gz", delete=False
        )
    ...
    def test_compress_tar_gz_file_wo_outfile(self):
        ...
        oufile = compress_tar_gz(infile, delete=False)
        # Check if the original file still exists
        self.assertTrue(os_path.isfile(infile))
        # Check if extracted file is equal to original one
        with TemporaryDirectory() as tempd:
    ...
    def test_compress_tar_gz_dir(self):
        oufile = compress_tar_gz(self.DATA_FOLDER, delete=False)
        ...
        with TemporaryDirectory() as tempd:
            ...
            extract_tar_gz(file=oufile, dir=tempd, delete=False)
            comp = dircmp(
                os_path.join(tempd, os_path.basename(self.DATA_FOLDER)),
                self.DATA_FOLDER,
            )
            self.assertListEqual([], comp.diff_files)
        remove(oufile)

    def test_compress_tar_gz_dir_delete(self):
        tempdir = mkdtemp()
        # Compress the temporary folder into a temporary tar.gz file
        outfile = compress_tar_gz(tempdir, delete=True)
    ...
    def test_download_and_unzip(self):
        with TemporaryDirectory() as tempd:
            ...
            self.assertEqual(2300, stat(os_path.join(tempd, "100l_file.txt")).st_size)
    ...
    def test_download_and_extract_tar_gz_member(self):
        ...
        with TemporaryDirectory() as tempd:
            ...
            for member in Test_File.DOWNLOAD_HASHES:
                ...
                if member == _member:
                    self.assertEqual(
                        Test_File.DOWNLOAD_HASHES[_member],
    ...
    def test_extract_tar_gz_file(self):
        with TemporaryDirectory() as tempd:
            ...
            self.assertEqual(
                sha256(
                    Path(os_path.join(tempd, "test_rpSBML.py")).read_bytes()
                ).hexdigest(),
    ...
    def test_extract_tar_gz(self):
        with TemporaryDirectory() as tempd:
            ...
            self.assertEqual(
                sha256(
                    Path(os_path.join(tempd, "test_Download.py")).read_bytes()
                ).hexdigest(),
    ...
    def test_read_tsv(self):
        f_json = open(os_path.join(self.DATA_FOLDER, "comp_xref.json"))
        self.assertListEqual(
            read_tsv(filename=os_path.join(self.DATA_FOLDER, "comp_xref.tsv")),
            json_load(f_json),
        )
        f_json.close()
